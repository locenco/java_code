...
class BufferedInputStream extends FilterInputStream {
    ...
    private void fill() throws IOException {
        ...
        //2.读完了buffer中的数据
            ...
            if (markpos > 0) {  /* can throw away early part of the buffer */
                ...
                // 作用是“将buffer中从markpos开始的数据”拷贝到buffer中(从位置0开始填充，填充长度是sz)。
                System.arraycopy(buffer, markpos, buffer, 0, sz);
                // 接着，将sz赋值给pos，即pos就是“被标记位置”到“buffer末尾”的数据长度。
                pos = sz;
                markpos = 0;
            //2.2 输入流没有被标记，重置
            } else if (buffer.length >= marklimit) {
                markpos = -1;   /*取消标记 buffer got too big, invalidate mark */
                pos = 0;        /* drop buffer contents */
            } else if (buffer.length >= MAX_BUFFER_SIZE) {
                throw new OutOfMemoryError("Required array size too large");
            } else {            /* grow buffer */
                // 新建一个字节数组nbuf。nbuf的大小是“pos*2”和“marklimit” MAX_BUFFER_SIZE 中较小的那个数。
     ...
     * 判断是否读完buffer中的数据，是，fill;否 return
     */
